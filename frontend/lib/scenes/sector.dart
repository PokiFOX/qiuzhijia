...
class SectorState extends State<SectorWidget> {
	...
	Widget build(BuildContext context) {
		List<TableRow> rows = [];
		for (var i = 0;i < tapah.sectorlist.length;i++) {
			var sector = tapah.sectorlist[i];
			rows.add(TableRow(
				children: [
					const TableCell(
					...
					TableCell(
						child: Center(
							child: Text(sector.id.toString(),),
						),
					),
					TableCell(
						child: Center(
							child: Text(sector.value,),
						),
					),
					TableCell(
						child: Row(
							...
							children: [
								GestureDetector(
									onTap: () async {
										String? result = await widget.InputDialog.show(context, title: "编辑大类", defaultText: sector.value,);
										if (result == null) return;
										await tapah.RequestEditSector(sector.id, result);
										await tapah.RequestSectorList();
										setState(() {});
									},
					...
					TableCell(
						child: Center(
							child: GestureDetector(
								onTap: () async {
									String? result = await widget.InputDialog.show(context, title: "添加大类", defaultText: "",);
									if (result == null) return;
