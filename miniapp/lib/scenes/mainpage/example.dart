...
class ExampleState extends State<ExampleWidget> with tapah.Callback {
	...
	bool isLoading = false, isFinish = false, isInitialLoading = false;
	...
	Widget buildExampleList() {
		...
		if (isInitialLoading) {
			...
			child = Padding(
				...
				child: ListView.separated(
					...
					itemCount: tapah.caselist.length + (isLoading ? 1 : 0),
					itemBuilder: (context, index) {
						if (index >= tapah.caselist.length) {
		...
		if (tapah.accountinfo == null) {
			return _wrapLoginOverlay(child);
		}
		return child;
	}
