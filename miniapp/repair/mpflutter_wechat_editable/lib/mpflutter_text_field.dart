...
/// Signature for the [MPFlutterTextField.buildCounter] callback.
...
class _TextFieldSelectionGestureDetectorBuilder
  ...
  _TextFieldSelectionGestureDetectorBuilder({
    required _MPFlutterTextFieldState state,
  })  : _state = state,
        super(delegate: state);

  final _MPFlutterTextFieldState _state;
  ...
  void onSingleTapUp(TapDragUpDetails details) {
    if (MPFlutter_Wechat_EditableInput.shouldUseWechatComponent()) {
      FocusScope.of(_state.context).requestFocus(_state._effectiveFocusNode);
    } else {
...
///
/// To integrate the [MPFlutterTextField] into a [Form] with other [FormField] widgets,
...
///
/// {@tool dartpad}
/// This example shows how to create a [MPFlutterTextField] that will obscure input. The
...
///    [MPFlutterTextField]. The [EditableText] widget is rarely used directly unless
...
class MPFlutterTextField extends StatefulWidget {
  ...
  /// [MPFlutterTextField.noMaxLength] then only the current length is displayed.
  ///
  ...
  const MPFlutterTextField({
    ...
    this.showConfirmBar,
        ...
        assert(maxLength == null ||
            maxLength == MPFlutterTextField.noMaxLength ||
            maxLength > 0),
  ...
  /// to [MPFlutterTextField.noMaxLength] then only the current character count is displayed.
  ///
  /// After [maxLength] characters have been input, additional input
  /// is ignored, unless [maxLengthEnforcement] is set to
  /// [MaxLengthEnforcement.none].
  ...
  /// which is evaluated after the supplied [inputFormatters], if any.
  ///
  /// This value must be either null, [MPFlutterTextField.noMaxLength], or greater than 0.
  /// If null (the default) then there is no limit to the number of characters
  /// that can be entered. If set to [MPFlutterTextField.noMaxLength], then no limit will
  ...
  /// "spinner" buttons that increment and decrement a value in the [MPFlutterTextField]
  ...
  ///
  /// The [mouseCursor] is the only property of [MPFlutterTextField] that controls the
  ...
  /// Default builder for [MPFlutterTextField]'s spell check suggestions toolbar.
  ///
  ...
  ///    parameter for which this is the default value for [MPFlutterTextField].
  ...
  static SpellCheckConfiguration inferAndroidSpellCheckConfiguration(
    ...
    return configuration.copyWith(
      misspelledTextStyle: configuration.misspelledTextStyle ??
          MPFlutterTextField.materialMisspelledTextStyle,
      spellCheckSuggestionsToolbarBuilder:
          configuration.spellCheckSuggestionsToolbarBuilder ??
              MPFlutterTextField.defaultSpellCheckSuggestionsToolbarBuilder,
    );
  }

  @override
  State<MPFlutterTextField> createState() => _MPFlutterTextFieldState();
...
class _MPFlutterTextFieldState extends State<MPFlutterTextField>
        ...
        .copyWith(
            ...
            hintMaxLines: widget.decoration?.hintMaxLines ??
                ((widget.minLines != null &&
                    ...
                    ? widget.minLines
                    : widget.maxLines),
            hintText: MPFlutter_Wechat_EditableInput.shouldUseWechatComponent()
                ? ""
  ...
  @override
  void didUpdateWidget(MPFlutterTextField oldWidget) {
  ...
  Widget build(BuildContext context) {
    ...
    switch (defaultTargetPlatform) {
      ...
      case TargetPlatform.windows:
        spellCheckConfiguration =
            MPFlutterTextField.inferAndroidSpellCheckConfiguration(
          widget.spellCheckConfiguration,
        );
    }
    ...
    Widget child = RepaintBoundary(
      child: UnmanagedRestorationScope(
        ...
        child: MPFlutter_Wechat_EditableInput(
          ...
          forceShowHintText: widget.decoration?.labelText == null,
